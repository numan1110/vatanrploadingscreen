...
class Util {
	...
	public static function log($type = 'access', $content = null, $force = false) {
		...
		$log_loc = APP_ROOT.$log_path.$type.'/'.$log;
	...
	public static function getSetting(...$keys) {
		...
		$settings = Database::conn()->select("SELECT `name`,`value` FROM `kload_settings`")->where($where, $keys)->execute();
		if ($settings) {
	...
	public static function updateSetting(array $settings, array $data, $csrf) {
		if (!User::validateCSRF($_SESSION['steamid'], $csrf) || User::isBanned($_SESSION['steamid'])) {
			Steam::Logout();
			return;
		}
